...
- (void)configTXPlayer {
    ...
    self.liveProgressTime = self.maxLiveProgressTime = self.hiddenProgressTime = 0;
    ...
    if (self.playerModel.multiVideoURLs.count > 0) {
        ...
        [self.netWatcher setNotifyBlock:^(NSString *msg) {
            ...
            if (strongSelf) {
                if (_videoIndex < self.playerModel.multiVideoURLs.count) {
...
- (void)seekToTime:(NSInteger)dragedSeconds {
    ...
    if (self.isLive) {
        ...
        int ret = [self.livePlayer seek:dragedSeconds+self.hiddenProgressTime];
        if (ret != 0) {
...
- (void)horizontalMoved:(CGFloat)value {
    ...
    [self.controlView playerDraggedTime:self.sumTime totalTime:totalMovieDuration];
}
...
- (CGFloat)getCurrentTime {
    if (self.isLive) {
        if (self.isShiftPlayback) {
            return self.shiftStartTime + self.liveProgressTime;
        }
        return self.maxLiveProgressTime;
    }
...
- (void)onControlView:(SuperPlayerControlView *)controlView progressSliderValueChanged:(UISlider *)slider {
    ...
    if (self.isLoaded) {
        ...
        if (totalTime > 0) { // 当总时长 > 0时候才能拖动slider
            [controlView playerDraggedTime:dragedSeconds totalTime:totalTime];
        }
    }
}
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (EvtID == PLAY_EVT_PLAY_BEGIN) {
            ...
            }
            {
                ...
                [self.controlView playerPlayableProgress:timeInterval / totalDuration];
            }
            return ;
...
- (void)onPlayEvent:(int)EvtID withParam:(NSDictionary *)param {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (EvtID == PLAY_EVT_PLAY_BEGIN) {
            ...
            [self.controlView playerShowTips:[NSString stringWithFormat:@"已切换为%@", _playerModel.multiVideoURLs[_videoIndex].title] delay:2];
            ...
            NSInteger hiddenProgress = 0;
            while (progress > MAX_SHIFT_TIME) {
                progress -= MAX_SHIFT_TIME;
                hiddenProgress += MAX_SHIFT_TIME;
            }
            self.liveProgressTime = progress;
            self.hiddenProgressTime = hiddenProgress;
            self.maxLiveProgressTime = MAX(self.maxLiveProgressTime, self.liveProgressTime);
            
            if (self.isShiftPlayback) {
                CGFloat sv = 1;
                if (self.maxLiveProgressTime > 0){
                    sv = (CGFloat)self.liveProgressTime / self.maxLiveProgressTime;
                }
...
- (void)reportPlay {
    if (self.reportTime == nil)
        return;
    if (self.isLive) {
        [DataReport report:@"superlive" param:@{@"usedtime":@(-[self.reportTime timeIntervalSinceNow])}];
    } else {
        [DataReport report:@"supervod" param:@{@"usedtime":@(-[self.reportTime timeIntervalSinceNow]), @"fileid":@(self.playerModel.fileId?1:0)}];
    }
    self.reportTime = nil;
}
